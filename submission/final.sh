...
# WRITE YOUR SOLUTION BELOW:
TREASURE_ADDR=$(bitcoin-cli -regtest -rpcwallet=treasurewallet getnewaddress "treasurewallet" bech32)
...
bitcoin-cli -regtest -rpcwallet=treasurewallet getbalance
...
# WRITE YOUR SOLUTION BELOW:
VERIFY_RESULT=$(bitcoin-cli -regtest -rpcwallet=btrustwallet verifymessage "$LEGACY_ADDR" "$SIGNATURE" "$SECRET_MESSAGE" | grep -oE '(true|false)' | head -1)
...
# WRITE YOUR SOLUTION BELOW:
NEW_TAPROOT_ADDR=$(bitcoin-cli -regtest -rpcwallet=btrustwallet getnewaddress "builderswallet" bech32m)
...
# WRITE YOUR SOLUTION BELOW:
INTERNAL_KEY=$(echo "$ADDR_INFO" | grep -oE '"pubkey":"[^"]+"' | cut -d '"' -f 4)
...
# WRITE YOUR SOLUTION BELOW:
TAPROOT_DESCRIPTOR=$(bitcoin-cli -regtest getdescriptorinfo "$SIMPLE_DESCRIPTOR" | grep -oE '"descriptor":"[^"]+"' | cut -d '"' -f 4)
...
# WRITE YOUR SOLUTION BELOW:
DERIVED_ADDR_RAW=$(bitcoin-cli -regtest derivepubkey "$TAPROOT_DESCRIPTOR" 0)
